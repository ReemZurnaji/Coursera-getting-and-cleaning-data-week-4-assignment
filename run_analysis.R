...
testset$activities <- as.character(t(testlables))
#Adding participents variable to both trainingset and testset:
trainingset$participents <- trainsubject[,1]
testset$participents <- testsubject[,1]
#Merging the data sets:
data <- rbind(trainingset, testset)
#Getting the Index for Mean and Standard Deviation features:
...
Tidydata <- aggregate(.~ activities+ participents,Tidydata,mean)
